...
def post_comment(body, token, repo_name, pr_id):
    g = Github(token)
    repo = g.get_repo(repo_name)
    pr = repo.get_pull(pr_id)
...
def main():
    ...
    for match in matches:
        ...
        resp += generate_comment(match)
        print(formatter.format(match, options.colored))
    
    post_comment(resp, options.token, options.repo_name, options.pr_id)
